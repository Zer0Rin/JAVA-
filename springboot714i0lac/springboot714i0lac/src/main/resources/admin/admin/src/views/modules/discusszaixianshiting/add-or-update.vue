<template>
	<div class="addEdit-block" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		>
			<template >
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="评分" prop="score">
					<el-input-number v-model="ruleForm.score" placeholder="评分" :disabled="ro.score"></el-input-number>
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else class="input" label="评分" prop="score">
...
export default {
    ...
    // 多级联动参数
		...
		info(id) {
			this.$http({
				url: `discusszaixianshiting/info/${id}`,
				method: "get"
			}).then(({ data }) => {
				if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discusszaixianshiting/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusszaixianshitingCrossAddOrUpdateFlag = false;
											this.parent.search();
