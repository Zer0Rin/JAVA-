<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yinyuexinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yinyuexinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yinyuexinxi','爬取数据')" type="success" @click="spider()">
						<span class="icon iconfont icon-shuju17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						爬取数据
					</el-button>
					<el-button class="btn18" v-if="isAuth('yinyuexinxi','生成数据')" type="success" @click="genDataClick()">
						<span class="icon iconfont icon-shuju17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						生成数据
					</el-button>

					<el-button class="btn18" v-if="isAuth('yinyuexinxi','词云展示')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						词云展示
					</el-button>
					<el-button class="btn18" v-if="isAuth('yinyuexinxi','作者歌曲数量统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						作者歌曲数量统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yinyuexinxi','收藏数对比')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						收藏数对比
					</el-button>
					<el-button class="btn18" v-if="isAuth('yinyuexinxi','播放数对比')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						播放数对比
					</el-button>
					<el-button class="btn18" v-if="isAuth('yinyuexinxi','评论数对比')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						评论数对比
					</el-button>
				</el-row>
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yinyuexinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="edit" v-if=" isAuth('yinyuexinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								改动
							</el-button>

							<el-button class="view" v-if="isAuth('yinyuexinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yinyuexinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shoucangChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="bofangChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="pinglunChart5" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
	...
	export default {
		...
		chartDialog3() {
			...
			this.$nextTick(()=>{
				...
				var shoucangChart3 = echarts.init(document.getElementById("shoucangChart3"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						shoucangChart3.setOption(option);
						  //根据窗口的大小变动图表
						window.onresize = function() {
							shoucangChart3.resize();
						};
					}
				});
			})
		...
		chartDialog4() {
			...
			this.$nextTick(()=>{
				var bofangChart4 = echarts.init(document.getElementById("bofangChart4"),'macarons');
				this.$http({
		...
		chartDialog5() {
			...
			this.$nextTick(()=>{
				...
				var pinglunChart5 = echarts.init(document.getElementById("pinglunChart5"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						pinglunChart5.setOption(option);
						  //根据窗口的大小变动图表
						window.onresize = function() {
							pinglunChart5.resize();
						};
					}
				});
			})
		...
		disscussListHandler(id,type) {
			this.$router.push({path:'/discussyinyuexinxi',query:{refid:id}});
		},
		yinyuexinxistatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'yinyuexinxi/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
		...
		genDataSave(){
			...
			this.$http({
				url: 'yinyuexinxi/batch/gen?recordNum=' + this.genDataForm.recordNum,
				method: 'post'
			}).then(res=>{
