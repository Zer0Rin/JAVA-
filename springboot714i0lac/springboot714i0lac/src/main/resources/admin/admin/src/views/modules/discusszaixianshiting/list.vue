		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discusszaixianshiting','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discusszaixianshiting','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discusszaixianshiting','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								改动
							</el-button>

							<el-button class="view" v-if="isAuth('discusszaixianshiting','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
...
import AddOrUpdate from "./add-or-update";
	export default {
		...
		getDataList() {
			...
			this.$http({
				url: "discusszaixianshiting/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
		...
		disscussListHandler(id,type) {
			this.$router.push({path:'/discussdiscusszaixianshiting',query:{refid:id}});
		},
		backClick(){
			history.back()
		},
		discusszaixianshitingstatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'discusszaixianshiting/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
		...
		async deleteHandler(id ,refid ) {
			...
			await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
				...
				await this.$http({
					url: "discusszaixianshiting/delete",
					method: "post",
					data: ids
				}).then(async ({ data }) => {
					if (data && data.code === 0) {
						this.$http({
							url: 'zaixianshiting/info/' + refid,
							method: "get",
						}).then(res=>{
