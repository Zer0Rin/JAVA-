<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yinlegoumai','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yinlegoumai','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yinlegoumai','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yinlegoumai','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="btn8" v-if="isAuth('yinlegoumai','发布音乐')" @click="yinlebofangCrossAddOrUpdateHandler(scope.row,'cross','否','是','[1]','已发布音乐！')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								发布音乐
							</el-button>
							<el-button class="edit" v-if=" isAuth('yinlegoumai','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yinlegoumai','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<yinlebofang-cross-add-or-update v-if="yinlebofangCrossAddOrUpdateFlag" :parent="this" ref="yinlebofangCrossaddOrUpdate"></yinlebofang-cross-add-or-update>
...
import yinlebofangCrossAddOrUpdate from "../yinlebofang/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				yinlebofangCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yinlebofangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.yinlebofangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yinlegoumai');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yinlebofangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yinlebofangCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				let params = {
					tradeno: row.dingdanbianhao,
					subject: row.songname,
					totalamount: row.jiage,
				}
				this.$http({
					url: "yinlegoumai/alipay",
					method: "get",
					params: params
				}).then(({ data }) => {
						if (data && data.code === 0) {
		...
		yinlegoumaistatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'yinlegoumai/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
