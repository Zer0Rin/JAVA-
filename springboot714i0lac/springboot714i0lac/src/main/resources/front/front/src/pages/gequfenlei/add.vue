    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="歌曲分类" prop="gequfenlei">
            <el-input v-model="ruleForm.gequfenlei" 
                placeholder="歌曲分类" clearable :disabled=" false  ||ro.gequfenlei"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`gequfenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('gequfenlei/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`gequfenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
