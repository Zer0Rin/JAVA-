...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="播放" prop="songfile">
            <file-upload
            tip="点击上传播放"
            action="file/upload"
            :limit="1"
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          songfile: [
            { required: true, message: '播放不能为空', trigger: 'blur' },
          ],
          yonghuming: [
          ],
          crossuserid: [
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yinlebofang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yinlebofang/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yinlebofang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
